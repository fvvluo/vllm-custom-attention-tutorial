#!/usr/bin/env python3
"""V3 paged-decode correctness: V3 vs V2 vs tutorial PyTorch reference vs tutorial Triton.

Same randomized paged KV cache builder as V2. Primary goal: V3 == V2 bit-identical
(cp.async load changes only WHEN K/V arrive in SMEM, not their values or the math).
Also reports V3 vs ref (strict 5e-3) and V3 vs triton (tutorial 2e-2).

Run:
  CUDA_VISIBLE_DEVICES=<gpu> /usr/bin/python -u \
    -m custom_backend.liuxiaochen_paged_decode.verify_paged_decode_v3 --suite <name>
"""
...
def build_paged(seq_lens, device, dtype, shuffle=False, pad_lead=False, seed=0):
    ...
    for sl in seq_lens:
        ...
        q = torch.randn(1, QH, D, device=device, dtype=dtype)
...
def run_case(name, seq_lens, device, scale=None, shuffle=False, pad_lead=False,
    ...
    out_v3 = torch.empty(num_tokens, QH, D, device=device, dtype=DT)
    qp, kp, vp, op = query.data_ptr(), key_cache.data_ptr(), value_cache.data_ptr(), out_v3.data_ptr()
    mem0 = torch.cuda.memory_allocated(device)
    paged_decode_v3(query=query, key_cache=key_cache, value_cache=value_cache,
                    output=out_v3, query_start_loc=qsl, seq_lens=seq_lens_t,
                    token_seq_idx=tsi, block_table=block_table, scale=scale,
                    split_size_tokens=split_size, max_seq_len=max_seq_len)
    mem1 = torch.cuda.memory_allocated(device)
    same_ptr = (query.data_ptr() == qp and key_cache.data_ptr() == kp and
                value_cache.data_ptr() == vp and out_v3.data_ptr() == op)

    v3f = out_v3.float(); v2f = out_v2.float(); trif = out_tri.float()

    def stats(a, b):
        ad = (a - b).abs(); return ad.max().item(), (ad / (b.abs() + 1e-6)).max().item()
    ma_v2, mr_v2 = stats(v3f, v2f)
    ma_ref, mr_ref = stats(v3f, ref)
    ma_tri, mr_tri = stats(v3f, trif)
    bit_identical = bool(torch.equal(out_v3, out_v2))
    nan = bool(torch.isnan(v3f).any()); inf = bool(torch.isinf(v3f).any())
    tol_ok = torch.allclose(v3f, ref, rtol=RTOL, atol=ATOL) and (not nan) and (not inf)
    ...
    print(f"    vs V2 : max_abs={ma_v2:.3e} max_rel={mr_v2:.3e} bit_identical={bit_identical}")
    ...
    print(f"    tutorial_tol(2e-2)={'PASS' if tol_ok else 'FAIL'}  "
          f"strict(5e-3 vs ref)={'PASS' if strict_ok else 'WARN'}  "
          f"V3==V2={'YES' if bit_identical else 'NO'}")
    return tol_ok
...
def main():
    ...
    if a.suite in ("basic", "all"):
        for sl in [1, 15, 16, 17, 63, 64, 65, 127, 128, 129, 512]:
